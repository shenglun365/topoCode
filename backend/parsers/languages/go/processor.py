# parsers/languages/go/processor.py
"""
Go 语言处理器

组合 Go AST 解析、符号提取、调用图提取、依赖图提取功能。

注意：各方法只处理 Go 语言的文件（.go），不影响其他语言。
"""
...
class GoASTParser(ASTParser):
    ...
    def parse_file(self, source_file_path: str, proj_id: int, proj_path: str) -> int:
        ...
        try:
            ...
            raise Exception(f"Failed to parse Go file {source_file_path}: {e}")
...
class GoSymbolExtractor(SymbolExtractor):
    ...
    def extract(self, proj_id: int) -> List[Dict[str, Any]]:
        ...
        # 写入数据库
        if global_defs:
            _save_symbols(db.graph_node, proj_id, global_defs, "go")

        return global_defs


class GoCallGraphExtractor(CallGraphExtractor):
    """Go 调用图提取器"""
    ...
    def extract(self, proj_id: int) -> List[Dict[str, Any]]:
        ...
        使用 extract_call_graph 的通用逻辑，但只处理 Go 文件
        """
        from parsers.extract_call_graph import extract_call_graph
        return extract_call_graph(proj_id, language='go')


class GoDependencyExtractor(DependencyExtractor):
    """Go 依赖图提取器"""
    ...
    def extract(self, proj_id: int) -> List[Dict[str, Any]]:
        ...
        使用 extract_dependency_graph 的通用逻辑，但只处理 Go 文件
        """
        from parsers.extract_dependency_graph import extract_dependency_graph
        return extract_dependency_graph(proj_id, language='go')
...
class GoLanguageProcessor(LanguageProcessor):
    ...
    def __init__(self):
        self._ast_parser = GoASTParser()
        self._symbol_extractor = GoSymbolExtractor()
        self._call_graph_extractor = GoCallGraphExtractor()
        self._dependency_extractor = GoDependencyExtractor()

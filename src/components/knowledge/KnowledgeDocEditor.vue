  ...
  <div class="kb-doc-editor">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 16px; border-bottom:1px solid var(--border); background:var(--bg-secondary); flex-shrink:0;">
      <div style="display:flex; align-items:center; gap:10px;">
        ...
        <span class="badge badge-green" style="font-size:8px;">{{ t('common.saved') }}</span>
      </div>
      <div style="display:flex; gap:6px; align-items:center;">
        ...
        <button class="btn btn-ghost btn-sm" @click="handleSave">
    ...
    <!-- 编辑模式 -->
    <div v-if="editMode === 'edit'" style="flex:1; overflow:hidden; display:flex;">
      <!-- 行号栏 -->
      <div
      ...
      >
        <div v-for="i in lineCount" :key="i" style="padding: 0 8px;">{{ i }}</div>
      </div>
      <!-- 代码编辑区 -->
      <textarea
        v-model="content"
    ...
    <!-- 浏览模式 -->
    <div v-else style="flex:1; overflow:auto; padding:20px 24px;">

...
async function sendVerifyCode() {
  ...
  try {
    ...
    toast.value = e.message || t('common.sendFailed')
...
async function verifyIdentity() {
  ...
  try {
    ...
    toast.value = t('auth.verifySuccess')
    bindStep.value = 'new'
  } catch (e: any) {
    toast.value = e.message || t('auth.verifyFailed')
  } finally {
...
async function confirmBind() {
  ...
  try {
    ...
    toast.value = t('auth.bindSuccess')
    resetBind()
    await auth.fetchProfile()
  } catch (e: any) {
    toast.value = e.message || t('auth.bindFailed')
  } finally {
...
async function saveUsername() {
  ...
  if (!name || name.length < 2 || name.length > 20) {
    toast.value = t('auth.usernameLengthError')
    setTimeout(() => { toast.value = '' }, 2000)
    return
  }
  try {
    ...
    toast.value = t('auth.usernameUpdated')
  } catch (e: any) {
    toast.value = e.message || t('auth.modifyFailed')
  }
  setTimeout(() => { toast.value = '' }, 2000)
}
...
function shareWeibo() {
  ...
  const title = encodeURIComponent(t('auth.shareTitle'))
...
<template>
  ...
  >
    ...
    <div class="info-list">
      ...
      <div class="info-row">
        ...
        >
          {{ auth.user.email }}
          <span class="badge badge-bound">{{ t('auth.bound') }}</span>
          <button
            class="btn-link"
            @click="startBind('email')"
          >{{ t('auth.rebind') }}</button>
        </span>
        <span
          v-else
          class="info-value"
        >
          <span class="text-unbound">{{ t('auth.unbound') }}</span>
          <button
            class="btn-link"
            @click="startBind('email')"
          >{{ t('auth.bind') }}</button>
        ...
        >
          <template v-if="isRebind && bindStep === 'verify'">
            ...
            <div class="verify-options">
              <label
                v-if="currentEmail"
                class="verify-option"
                :class="{ active: bindVerifyTarget === 'email' }"
              >
                <input
                  v-model="bindVerifyTarget"
                  type="radio"
                  value="email"
                > {{ currentEmail }}
              </label>
              <label
                v-if="currentPhone"
                class="verify-option"
                :class="{ active: bindVerifyTarget === 'phone' }"
              >
                <input
                  v-model="bindVerifyTarget"
                  type="radio"
            ...
            >
              ...
              <button
                ...
                :disabled="bindVerifySending || bindVerifySent"
                @click="sendVerifyCode"
              >
                {{ bindVerifySending ? '...' : bindVerifySent ? `${bindVerifyCountdown}s` : t('auth.sendCode') }}
              </button>
            </div>
            <div
              class="code-row"
            ...
            >
              <button
                ...
                :disabled="!bindVerifyCode || bindVerifying"
                @click="verifyIdentity"
              >
                {{ bindVerifying ? t('auth.verifying') : t('auth.verifyIdentity') }}
              </button>
              <button
                class="btn btn-sm btn-ghost code-btn"
                @click="resetBind"
          ...
          <template v-else-if="bindStep === 'new'">
            <div class="code-row">
              ...
              <button
                class="btn btn-sm btn-ghost code-btn"
                :disabled="bindSending || bindSent"
                @click="sendNewCode"
              >
                {{ bindSending ? '...' : bindSent ? `${bindCountdown}s` : t('auth.sendCode') }}
              </button>
            </div>
            <div
              class="code-row"
            ...
            >
              <button
                class="btn btn-sm btn-primary code-btn"
                :disabled="bindLoading || !bindAccount || !bindCode"
                @click="confirmBind"
              >
                {{ bindLoading ? '...' : t('auth.confirmBind') }}
              </button>
              <button
                class="btn btn-sm btn-ghost code-btn"
                @click="resetBind"
      ...
      <div class="info-row">
        ...
        <span
          ...
          class="info-value"
        >
          <span class="text-unbound">{{ t('auth.passwordNotSet') }}</span>
          <router-link
            to="/reset-password?set=1"
            class="btn-link"
          >{{ t('auth.setPassword') }}</router-link>
        </span>
      </div>
      <div class="info-row">
  ...
  <div
    v-else
    class="profile-tab unauthenticated"
  >
    <p class="login-hint">
      {{ t('auth.loginHint', '登录后可查看个人资料和下载资源') }}
    </p>
    <div class="actions">
      <router-link
        to="/login"

...
import type { AnimationState, AnimationInstruction } from '../core/types';
...
/** 反序列化状态 */
function deserializeStates(data: any[]): AnimationState[] {
  return data.map((item) => ({
    ...item,
    nodes: new Map(item.nodes),
    edges: new Map(item.edges),
    groups: item.groups ? new Map(item.groups) : undefined,
  }));
}

...
import type { FunctionRegistry, TopoFunction, TopoRuntime } from './PluginInterface';
...
class FunctionRegistryImpl implements FunctionRegistry {
  ...
  call(name: string, args: any[], runtime: TopoRuntime): any {
    const fn = this.functions.get(name);
    if (!fn) {
      throw new Error(`Function '${name}' not found`);
    }
    return fn.execute(args, runtime);
  }
}

...
export class TopoSemanticChecker {
  ...
  private checkNodes(nodes: any[]): void {
    for (const node of nodes) {
      if (!node.id) {
        this.errors.push({ message: 'Node missing id' });
        continue;
      }

      if (this.nodeIds.has(node.id)) {
        this.errors.push({ message: `Duplicate node id: ${node.id}` });
      }
  ...
  private checkEdges(edges: any[]): void {
    for (const edge of edges) {
      ...
      if (this.edgeIds.has(edgeId)) {
        this.warnings.push({ message: `Duplicate edge: ${edgeId}` });
      }
  ...
  private checkGroups(groups: any[]): void {
    for (const group of groups) {
      if (!group.id) {
        this.errors.push({ message: 'Group missing id' });
        continue;
      }

      if (this.groupIds.has(group.id)) {
        this.errors.push({ message: `Duplicate group id: ${group.id}` });
      }

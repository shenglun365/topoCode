...
def _update_progress(server, task_id: str, run_id: str,
                     current: int, total: int):
    """
    更新进度: SQLite + ZMQ PUB 推送

    Args:
        server: RPCHandler 实例（有 publish 方法）
        task_id: 任务 ID
        run_id: 运行 ID
        current: 当前处理到第几个文件
        total: 总文件数
...
def _do_parse(server, task_id: str, run_id: str, start_time: float) -> Dict[str, Any]:
    """
    线程池中的阻塞执行函数 — 6 步分析流程

    Args:
        server: RPCHandler 实例
        task_id: 任务 ID
        run_id: 运行 ID
        start_time: 开始时间戳
    ...
    from .parsers_new.language_registry import LanguageRegistry
    ...
    for lang, file_list in files_by_lang.items():
        processor = LanguageRegistry.get_processor(lang)
        if not processor:
            _log(f"语言 '{lang}' 没有注册的处理器，跳过 {len(file_list)} 个文件")
            skipped += len(file_list)
            continue

        _log(f"处理语言 {lang}: {len(file_list)} 个文件")
        ast_parser = processor.get_ast_parser()
        ...
        for f in file_list:
            ...
            try:
                node_count = ast_parser.parse_file(
                    source_file_path=f["file_path"],
                    file_id=f["id"],
                ...
                if processed % PROGRESS_INTERVAL == 0:
                    _update_progress(server, task_id, run_id, processed, total)
    ...
    # ==================== Step 2: 符号提取 ====================
    _log("Step 2: 符号提取开始")
    for lang in list(language_stats.keys()):
        if should_stop(task_id):
            break
        processor = LanguageRegistry.get_processor(lang)
        if not processor:
            continue
        try:
            symbols = processor.extract_symbols(task_id, analysis_store)
            _log(f"{lang}: 提取 {len(symbols)} 个符号")
        except Exception as e:
            _log(f"{lang} 符号提取失败: {e}")
            raise
    ...
    for lang in list(language_stats.keys()):
        ...
        if not processor:
            continue
        try:
            edges = processor.extract_call_graph(task_id, analysis_store)
            total_call_edges += len(edges)
            _log(f"{lang}: 提取 {len(edges)} 条调用边")
        except Exception as e:
            _log(f"{lang} 调用图提取失败: {e}")
            raise
    ...
    for lang in list(language_stats.keys()):
        ...
        if not processor:
            continue
        try:
            deps = processor.extract_dependency_graph(task_id, analysis_store)
            total_dep_edges += len(deps)
            _log(f"{lang}: 提取 {len(deps)} 条依赖边")
        except Exception as e:
            _log(f"{lang} 依赖图提取失败: {e}")
            raise
    ...
    # 更新最终进度
    _update_progress(server, task_id, run_id, total, total)
...
async def _execute_task(server, task_id: str, run_id: str, start_time: float):
    """
    ...
    try:
        result = await loop.run_in_executor(
            parse_executor,
            _do_parse,
            server, task_id, run_id, start_time,
        )
        ...
        if result.get("stopped"):
            task_store.update_task_status(task_id, "stopped", progress=result.get("progress", 0))

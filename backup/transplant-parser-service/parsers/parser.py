...
解析结果存储到 MongoDB 的 base_node 集合。
...
# 导入配置文件
from config.parser_config import MAX_FILE_SIZE, MAX_FILE_SIZE_HUMAN, should_skip_file
from config.parse_project_config import load_or_create_default_config
...
from parsers.languages.code_parser.registry import get_language_config
from parsers.languages.code_parser.lang_parser_conf import detect_language

logger = logging.getLogger(__name__)


def source_code_to_ast(source_file_path: str, proj_id: int, proj_path: str) -> int:
    """
    将源代码解析为 AST，并将结果存入 MongoDB

    Args:
        source_file_path: 源代码文件路径（绝对路径）
        proj_id: 项目 ID（int 类型）
    ...
    Raises:
        Exception: 解析失败时抛出异常，由 Celery 任务捕获
    """
    try:
        # === Step 1: 语言检测 ===
        ...
        # === Step 4: 初始化数据库连接（提前初始化，用于跳过文件的记录） ===
        mongo_client = get_mongo_client()
        db = mongo_client.get_database()
        proj_info_coll = db.proj_info
        ast_nodes_coll = db.base_node

        # 检查文件大小
        file_size = source_path.stat().st_size
        should_skip, skip_reason = should_skip_file(file_size)
        if should_skip:
            logger.warning(f"⚠️  Skipping large file: {source_file_path} ({file_size / (1024*1024):.2f}MB) - {skip_reason}")
            # 记录到 proj_info 但标记为已跳过
            proj_info_coll.update_one(
                {"proj_id": proj_id, "file_path": os.path.relpath(source_file_path, proj_path)},
                {"$set": {
        ...
        # === 新增：检查项目配置文件 ===
        # 加载项目配置（使用缓存避免重复读取）
        if not hasattr(source_code_to_ast, '_project_config_cache'):
            source_code_to_ast._project_config_cache = {}

        if proj_path not in source_code_to_ast._project_config_cache:
            source_code_to_ast._project_config_cache[proj_path] = load_or_create_default_config(proj_path)

        project_config = source_code_to_ast._project_config_cache[proj_path]

        # 检查文件是否应该解析
        rel_path = os.path.relpath(source_file_path, proj_path)
        if not project_config.should_include_file(rel_path, language_name):
            logger.debug(f"⚠️  Skipping file (excluded by config): {rel_path}")
            # 记录到 proj_info 但标记为已跳过
            proj_info_coll.update_one(
                {"proj_id": proj_id, "file_path": rel_path},
                {"$set": {
                    "parse_status": "skipped_by_config",
                    "file_size": file_size
                }},
                upsert=True
            )
            return 0  # 返回 0 表示跳过（不报错）

        src_content = source_path.read_bytes()
        mtime = os.path.getmtime(source_path)
        hashcode = hashlib.sha256(src_content).hexdigest()
        file_name = source_path.name
        file_path = rel_path

        logger.info(f"Processing file: {file_path}, proj_id={proj_id}, lang={language_name}, size={file_size} bytes")

        # === Step 5: 获取或创建 file_id ===
        file_id = _get_or_create_file_id(
            ...
            hashcode=hashcode,
            mtime=mtime,
            ast_nodes_coll=ast_nodes_coll
        )
        
        if file_id == 0:
            logger.info(f"File unchanged, skipping AST parsing: {file_path}")
            return 0
        ...
        logger.info(f"Parsed {total_nodes} nodes from {file_path}")

        # === Step 7: 批量插入 MongoDB ===
        inserted_total = _batch_insert_ast_nodes(
            ast_nodes_coll=ast_nodes_coll,
            proj_id=proj_id,
            file_id=file_id,
            nodes=nodes_final
        ...
        logger.info(f"✅ Successfully inserted {inserted_total} AST nodes for file_id={file_id} (proj_id={proj_id})")
        return inserted_total

    except Exception as e:
        logger.exception(f"❌ Fatal error in source_code_to_ast for {source_file_path}, proj_id={proj_id}: {e}")
        raise  # 确保异常传播到 Celery，便于监控和重试
...
def _get_or_create_file_id(
    ...
    如果文件已存在且未变化，返回 0 表示跳过解析
    如果文件已存在但已变化，更新记录并返回原 file_id
    如果文件不存在，创建新记录并返回新 file_id

    Returns:
        file_id: 文件 ID，如果文件未变化返回 0
    """
    # 查找现有记录
    existing = proj_info_coll.find_one({
        "proj_id": proj_id,
        "file_path": file_path
    }, {"file_id": 1, "hashcode": 1, "mtime": 1, "_id": 1})

    if existing:
        file_id = existing["file_id"]

        # 检查文件是否变化
        if existing.get("hashcode") == hashcode and existing.get("mtime") == mtime:
            # 检查 AST 节点是否存在（防御性检查）
            ast_count = ast_nodes_coll.count_documents({"proj_id": proj_id, "file_id": file_id})
            if ast_count > 0:
                logger.info(f"File unchanged, reusing file_id: {file_id} (AST nodes: {ast_count})")
                return 0  # 文件未变化且 AST 存在，跳过
            else:
                logger.warning(f"File unchanged but AST nodes missing, re-parsing: {file_path} (file_id={file_id})")
                # 继续解析，不返回 0

        # 文件已变化或 AST 缺失，更新记录
        result = proj_info_coll.update_one(
            {"_id": existing["_id"]},
            {"$set": {"hashcode": hashcode, "mtime": mtime}}
        )

        # 清理旧 AST 数据
        ast_nodes_coll.delete_many({"proj_id": proj_id, "file_id": file_id})

        logger.info(f"File changed/missing AST, updated record: matched={result.matched_count}, modified={result.modified_count}")
        return file_id
    else:
        # 新文件，分配新 file_id
        max_record = proj_info_coll.find_one(
            {"proj_id": proj_id, "file_id": {"$exists": True}},
            sort=[("file_id", -1)]
        )
        file_id = (max_record["file_id"] + 1) if max_record and "file_id" in max_record else 1

        # 检测文件语言
        from parsers.languages.code_parser.lang_parser_conf import detect_language
        file_lang = detect_language(file_path) or detect_language(file_name)

        insert_result = proj_info_coll.insert_one({
            "proj_id": proj_id,
            "file_id": file_id,
            "file_path": file_path,
            "file_name": file_name,
            "file_lang": file_lang,  # 新增字段：文件语言
            "hashcode": hashcode,
            "mtime": mtime
        })
        ...
        logger.info(f"Inserted new file record, _id={insert_result.inserted_id}, file_id={file_id}, lang={file_lang}")
        return file_id


def _batch_insert_ast_nodes(
    ast_nodes_coll,
    proj_id: int,
    file_id: int,
    nodes: List[Dict],
    ...
    """
    批量插入 AST 节点到 MongoDB

    使用 bulk_write + ordered=False 优化大批量插入性能
    
    Args:
        ast_nodes_coll: MongoDB 集合
        proj_id: 项目 ID
        file_id: 文件 ID
        nodes: AST 节点列表
        batch_size: 批次大小（默认 5000）
    ...
    for i in range(0, total_nodes, batch_size):
        ...
        for obj in batch:
            doc = {
                ...
                "name": obj["name"],
                "op": obj["op"],
                "refs": [] if obj["type"] == "translation_unit" else obj["refs"],
                "node_id": obj["node_id"],
                "scope_node_id": obj["scope_node_id"],
                "def_node_id": obj["def_node_id"],
                "content_size": obj["content_size"],
            }
            documents.append(doc)

        try:
            # 使用 bulk_write + ordered=False 提高性能
            # ordered=False 允许 MongoDB 并行插入，性能更好
            operations = [InsertOne(doc) for doc in documents]
            result = ast_nodes_coll.bulk_write(operations, ordered=False)
            
            # BulkWriteResult 没有 inserted_ids
            # 使用 inserted_count 属性（如果可用）或回退到操作数
            # 注意：对于纯插入操作，result.inserted_count 可能不可用
            # 最可靠的方法是假设所有操作都成功（MongoDB 会抛出异常如果失败）
            inserted_count = len(operations)
            inserted_total += inserted_count
            
            logger.info(f"✅ Batch inserted {inserted_count} AST nodes, batch {i//batch_size + 1}/{(total_nodes + batch_size - 1)//batch_size}")
            
        except Exception as e:
            logger.exception(f"❌ Failed to insert batch starting at index {i}: {e}")
            raise  # 重新抛出，确保 Celery 标记任务失败

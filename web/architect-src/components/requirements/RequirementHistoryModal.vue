...
const props = defineProps<{ open: boolean; activeConvId?: string }>()
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await requirementService.conversations()
  } catch {
    items.value = []
  } finally {
    loading.value = false
    ...
    <div class="panel w-full max-w-lg max-h-[70vh] overflow-hidden flex flex-col">
      <div class="panel-header shrink-0">
        <span class="flex items-center gap-2">
          <ClipboardDocumentListIcon class="w-4 h-4 text-ctp-blue" />{{ t('requirement.history.title') }}
        </span>
        <button
        ...
        >
          ...
          >
            ...
            <div class="flex items-center gap-1 shrink-0">
              ...
              >
                <CheckIcon class="w-3 h-3 inline" />{{ t('requirement.history.imported') }}
              </span>
              <button
                v-else
                ...
                @click="onImport(item)"
              >
                <ArrowDownTrayIcon class="w-3 h-3" />{{ t('requirement.history.import') }}
              </button>
              <button
                class="btn btn-sm btn-ghost !py-1 text-ctp-red"
                :disabled="loadingDetail"
                :title="t('requirement.history.delete')"
                @click="onDelete(item)"
              >

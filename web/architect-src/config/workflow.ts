...
export const WORKFLOW_STAGES: WorkflowStageDef[] = [
  ...
  },
  {
    key: 'accept',
    labelKey: 'accept',
    route: { path: '/workbench/unit-test' },
    matches: (ctx, path) =>
      path === '/workbench/unit-test'
      || (path === '/workbench/execute' && (ctx.acceptingCount > 0 || ctx.doneTaskCount > 0)),
    isDone: (ctx: WorkflowStageCtx) => ctx.doneTaskCount > 0 || ctx.passedTestCount > 0,
  },
]

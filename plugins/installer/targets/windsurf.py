...
class WindsurfTarget(AgentTarget):
    id = "windsurf"
    name = "Windsurf"

    def detect(self) -> bool:
        return bool((Path.home() / ".windsurf" / "mcp.json").exists()
                    or (Path(".windsurf") / "mcp.json").exists())

    def install(self, project_root: str | None = None, global_: bool = False) -> str | None:
        if global_:
            config_path = Path.home() / ".windsurf" / "mcp.json"
        else:
            config_path = Path(project_root or ".") / ".windsurf" / "mcp.json"
    ...
    def describe_paths(self) -> list[str]:
        return [str(Path.home() / ".windsurf" / "mcp.json")]

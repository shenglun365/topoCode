...
def extract_graph_symbols(adapter: SQLiteAdapter) -> int:
    """从 graph_node 已有的 symbol 记录提取 func_name/class_name (新 Query 管线)

    新 pipeline (parse_with_queries) 已将符号写入 graph_node (symbol_node_type='symbol'),
    此函数读取并补充旧格式的 func_name/class_name 记录, 保持下游兼容性.
    ...
    for sym in symbols:
        ...
        if kind in ('function', 'method', 'variable', 'parameter'):
            records.append({
                "symbol_node_type": "func_name",
                "file_id": sym.get('file_id'),
                "func_name": name,
                "def_file_id": sym.get('file_id'),
                "start_line": str(sym.get('start_line', 0)),
                "end_line": str(sym.get('end_line', 0)),
            })
    ...
    if records:
        ...
        logger.info(f"[extract_graph_symbols] 写入 {len(records)} 条: func_name={sum(1 for r in records if r['symbol_node_type']=='func_name')}, class_name={sum(1 for r in records if r['symbol_node_type']=='class_name')}")

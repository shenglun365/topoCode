...
def _build_callee_map(adapter: SQLiteAdapter) -> dict[str, dict]:
    """从 graph_node 的 symbol 记录构建被调用者映射: name → {file_id, node_id}

    查询所有 symbol_node_type='symbol' 的记录, 按 name 去重 (同名取第一条).
    """
    symbols = adapter.find_graph(symbol_node_type='symbol')
    callee_map: dict[str, dict] = {}
    for sym in symbols:
        name = sym.get('name', '') or ''
        if not name:
            continue
        if name not in callee_map:
            callee_map[name] = {
                'file_id': sym.get('file_id', '') or '',
                'node_id': sym.get('id', '') or '',
            }
    return callee_map


def extract_call_edges_from_refs(adapter: SQLiteAdapter) -> list[dict]:
    """从 graph_node 已有的 reference 记录中提取调用边 (新 Query 管线)

    parse_with_queries 已将调用引用写入 graph_node (symbol_node_type='reference', kind='call'),
    此函数读取并生成 call_relation 记录供 community_analysis 使用.
    ...
    Returns:
        调用边列表
    """
    refs = adapter.find_graph(symbol_node_type='reference')
    call_refs = [r for r in refs if r.get('kind') == 'call']

    if not call_refs:
        return []

    callee_map = _build_callee_map(adapter)
    total = len(call_refs)
    resolved = 0

    call_edges = []
    for ref in call_refs:
        callee_name = ref.get('name', '') or ''
        callee_info = callee_map.get(callee_name)
        if callee_info:
            resolved += 1
        call_edges.append({
            "symbol_node_type": "call_relation",
            "caller_file_id": ref.get('file_id', ''),
            "caller_func_name": ref.get('scope', ''),
            "callee_name": callee_name,
            "callee_file_id": callee_info['file_id'] if callee_info else '',
            "callee_node_id": callee_info['node_id'] if callee_info else '',
        })

    if call_edges:
        adapter.insert_graph(call_edges)
        logger.info(f"[extract_call_edges_from_refs] 写入 {len(call_edges)} 条调用边 (resolved {resolved}/{total})")
